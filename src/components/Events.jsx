  ...
  return (
    <div id="etkinlikler" className="bg-[#0E2A44] flex flex-col items-center py-20 md:py-24 pb-28 md:pb-32">
      ...
      <div className="mb-12 md:mb-16 text-center px-4">
        ...
        />
         {/* Or a text title: 
         <h2 className="text-5xl md:text-6xl font-bold text-white tracking-tight">Etkinlikler</h2>
         <p className="text-lg text-gray-300 mt-2">En son duyurular ve yarışmalar.</p> 
         */}
      </div>
      ...
      {currentEvent && (
        <div className="w-full max-w-4xl px-4 mb-12 md:mb-16">
          <div className="bg-gradient-to-br from-[#2269A9] to-[#1A5083] p-6 sm:p-8 rounded-xl shadow-2xl text-white transition-all duration-500 ease-in-out transform hover:scale-[1.02]">
            <h2 className="text-3xl sm:text-4xl font-bold mb-3 sm:mb-4 tracking-tight">{currentEvent.title}</h2>
            <div className="flex items-center text-gray-200 mb-4 text-sm sm:text-base">
              ...
              <span>
                {new Date(currentEvent.date.split(' ')[0].split('-').reverse().join('-') + 'T' + currentEvent.date.split(' ')[1]).toLocaleDateString("tr-TR", {
                  year: 'numeric', month: 'long', day: 'numeric', hour: '2-digit', minute: '2-digit'
                })}
              </span>
              {!currentEvent.isActive && <span className="ml-3 text-xs bg-yellow-500 text-black font-semibold px-2 py-0.5 rounded-full">(Geçmiş Etkinlik)</span>}
            </div>
            ...
            {currentEvent.formUrl && currentEvent.isActive && (
              <a
                ...
                {events.map((event, index) => (
                  ...
                  >
                    ...
                    <p className={`text-xs ${index === activeEventIndex ? "text-gray-200" : "text-gray-400"}`}>
                      {new Date(event.date.split(' ')[0].split('-').reverse().join('-') + 'T' + event.date.split(' ')[1]).toLocaleDateString("tr-TR", { month: 'short', day: 'numeric' })}
                    </p>
                  </div>
                ))}
              </div>
